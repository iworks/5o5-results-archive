...
class OPI_Jobs_Cookie_Notice extends OPI_Jobs {
	/**
	 * Module option name.
	 *
	 * @var string
	 */
	protected $option_name = '_opi_cookie_notice';
	...
	public function __construct() {
		parent::__construct();
		add_action( 'wp_localize_script_int505_archive_theme', array( $this, 'add_cookie_data' ) );
		add_action( 'wp_footer', array( $this, 'add_cookie_notice' ), PHP_INT_MAX );
		add_action( 'wp_ajax_int505_archive_cookie_notice', array( $this, 'save_user_meta' ) );
		add_action( 'wp_ajax_nopriv_int505_archive_dismiss_visitor_notice', array( $this, 'dismiss_visitor_notice' ) );
	}
	...
	private function set_data() {
		...
		$this->data = array(
			...
			'text'    => sprintf(
				__( 'We use cookies and similar technologies to provide services and to gather information for statistical and other purposes. You can change the way you want the cookies to be stored or accessed on your device in the settings of your browser. If you do not agree, change the settings of your browser. For more information, refer to our %s.', 'opi-jobs' ),
				sprintf(
					'<a href="%s">%s</a>',
					get_privacy_policy_url(),
					_x( 'Privacy Policy', 'in cookie message, mayby propoer form', 'opi-jobs' )
				)
			),
		);
	}
	...
	public function add_cookie_notice() {
		...
		$content .= sprintf(
			'<div class="cookie-notice-container"><span id="opi-jobs-cn-notice-text" class="text">%s</span>',
			$this->data['text']
		);
		// Data.
		$content .= sprintf(
			'<span><a href="#" class="button opi-jobs-cn-set-cookie" aria-label="%s">%s</a></span>',
			esc_attr__( 'Close cookie information.', 'opi-jobs' ),
			esc_html__( 'Close cookie information', 'opi-jobs' )
		);
		$content .= '</div>';
		$content .= '</div>';
		echo apply_filters( 'int505_archive_cookie_notice_output', $content, $this->data );
	}
